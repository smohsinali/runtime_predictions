...
def plot_prediction(x_features, x_datasize, y_true, y_predicted, data_test, data_train):
    ...
    for i, predicted in enumerate(y_predicted):
        ...
        row.append(pd.DataFrame({data_test: [y_true[-1], y_pred[-1], 100 * (abs(y_true[-1] - y_pred[-1])) / y_true[-1]]},
    ...
    plt.savefig("results/mc_" + data_test + "_pred_n.png", dpi=200)
    plt.close()
...
def plot_get_hp_table(x_features, x_datasize, y_true, y_predicted, hp_values, data_test, data_train):
    '''
